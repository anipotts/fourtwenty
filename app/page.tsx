...
export default function Home() {
  ...
  return (
    <main className={`h-screen w-screen flex flex-col items-center justify-center px-6 relative transition-colors duration-300 ${dark ? 'bg-black' : 'bg-cream'}`}>
      <div className="flex flex-col items-center justify-center flex-1">
        ...
        {verified === null && (
          <div className="flex flex-col items-center">
            <h1 className={`text-2xl sm:text-3xl font-bold mb-8 text-center animate-fade-in animate-delay-100 ${dark ? 'text-emerald-400' : 'text-leaf-dark'}`}>
              Are you 21?
            </h1>
            <div className="flex gap-4 animate-fade-in animate-delay-200">
              <button
                onClick={() => setVerified(true)}
                className={`px-8 py-3 font-bold rounded-full text-lg hover:scale-105 active:scale-95 transition-all duration-150 ${dark ? 'bg-emerald-400 text-black hover:bg-emerald-300' : 'bg-leaf text-cream hover:bg-leaf-dark'}`}
              >
                Yes
              </button>
              <button
                onClick={() => setVerified(false)}
                className={`px-8 py-3 border-2 font-bold rounded-full text-lg hover:scale-105 active:scale-95 transition-all duration-150 ${dark ? 'border-emerald-400 text-emerald-400 hover:bg-emerald-400 hover:text-black' : 'border-leaf text-leaf hover:bg-leaf hover:text-cream'}`}
              >
                No
              </button>
            </div>
        ...
        {verified === true && (
          <div className="flex flex-col items-center text-center animate-scale-in">
            <p className={`text-xl sm:text-2xl font-medium leading-relaxed ${dark ? 'text-emerald-400' : 'text-leaf-dark'}`}>
              Keep an eye out for
              <br />
              <span className={`text-3xl sm:text-4xl font-bold ${dark ? 'text-emerald-300' : 'text-leaf'}`}>
                4/20 in NYC.
              </span>
              <br />
              2026.
        ...
        {verified === false && (
          <div className="flex flex-col items-center text-center animate-scale-in">
            <p className={`text-xl sm:text-2xl font-medium ${dark ? 'text-emerald-400' : 'text-leaf-dark'}`}>
